...
class CInputManager
{
	bool m_bPressedKeys[KEYS_MAX];
	int  m_iMousePos[2];
	int  m_iScrollWheelState;

public:
	void CaptureMouseMove(long lparam);

	// Registers a key press from wndproc
	bool RegisterKeyPress(VirtualKeyEvents_t keyEvent, VirtualKeys_t key);

	// Checks if the key is pressed
	bool IsKeyPressed(int key);
	bool IsKeyPressedToggle(int key);

	// Returns the first found key pressed, or KEY_NONE if none are
	VirtualKeys_t IsAnyKeyPressed();

	// Returns the last scroll state and resets it to 0
	int GetScrollState();

	// Returns the key's name
	const char* GetKeyName(VirtualKeys_t key);

	// Returns the first found currently pressed key
	char GetPressedChar(VirtualKeys_t* pressed_key = nullptr);

	// Check if a key is valid
	inline bool IsValidKey(VirtualKeys_t key)
	{
		return key > KEYS_NONE && key < KEYS_MAX;
	}

	// Get cursor position.
	inline void InputGetCursorPos(int& x, int& y)
	{
		x = m_iMousePos[0];
		y = m_iMousePos[1];
	}
